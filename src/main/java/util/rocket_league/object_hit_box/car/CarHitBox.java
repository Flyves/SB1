package util.rocket_league.object_hit_box.car;
...
public class CarHitBox {
    ...
    public CarHitBox(Vector3 centerPositionOfCar, rlbot.flat.Vector3 centerOfMassOffset, BoxShape boxShape, Vector3 frontOrientation, Vector3 roofOrientation) {
        this.centerPositionOfHitBox = centerPositionOfCar.plus(new Vector3(centerOfMassOffset).scaled(-1, 1, 1).rotateFromOrientationReferenceTo(new Orientation(frontOrientation, roofOrientation)));
    ...
    public CarHitBox(Vector3 centerPosition, Vector3 boxSize) {
    ...
    private CarHitBox(Vector3 centerPosition, Vector3 boxSize, Orientation orientation) {
        this.centerPositionOfHitBox = centerPosition;
    ...
    public CarHitBox generateHypotheticalHitBox(Vector3 hypotheticalPosition, Orientation hypotheticalOrientation) {
        return new CarHitBox(hypotheticalPosition, cornerPosition, hypotheticalOrientation);
    }

    public CarHitBox generateHypotheticalHitBox(Vector3 hypotheticalPosition) {
        return new CarHitBox(hypotheticalPosition, cornerPosition, new Orientation(frontOrientation, roofOrientation));
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof CarHitBox)) {
            return false;
        }
        return ((CarHitBox)obj).centerPositionOfHitBox.minus(this.centerPositionOfHitBox).magnitudeSquared() < 0.1
        && ((CarHitBox)obj).cornerPosition.minus(this.cornerPosition).magnitudeSquared() < 0.1
        && ((CarHitBox)obj).frontOrientation.minus(this.frontOrientation).magnitudeSquared() < 0.1
        && ((CarHitBox)obj).roofOrientation.minus(this.roofOrientation).magnitudeSquared() < 0.1;
    }
    ...
    private Vector3 getLocal(Vector3 globalPoint) {
        return globalPoint.minus(centerPositionOfHitBox).rotateToOrientationReferenceFrom(new Orientation(frontOrientation, roofOrientation));
    }
